...
const VSIAZ = '/vsiaz/'
const CONTAINER_NAME = 'customer-uploads'
...
export default async function (context, config) {
  ...
  const { blobName } = config
  config.fileConfig = {
    fileLocation: blobName,
    fileExtension: path.extname(blobName),
    fileDirectory: path.dirname(blobName),
    filename: path.basename(blobName, path.extname(blobName))
  }
  ...
  const landBoundaryConfig = {
    ...
    inputLocation: `${config.fileConfig.fileExtension === '.zip' ? '/vsizip' : ''}/vsiaz_streaming/${CONTAINER_NAME}/${config.fileConfig.fileLocation}`,
    ...
    outputLocation: `${VSIAZ}${CONTAINER_NAME}/${tmpGeoJsonBlobName}`,
    reprojectedOutputLocation: `${VSIAZ}${CONTAINER_NAME}/${tmpReprojectedGeoJsonBlobName}`,
