...
describe('BNG data extractor test', () => {
  it('should extract a valid metric v4.1', async () => {
    ...
    expect(response.d3OffSiteHabitatEnhancement[0]['Proposed habitat']).toEqual('Cereal crops')
    expect(response.d3OffSiteHabitatEnhancement[0]['Proposed Broad Habitat']).toEqual('Cropland')
    ...
    expect(response.d3OffSiteHabitatEnhancement[0]['Habitat units delivered']).toEqual('')
    expect((response.d3OffSiteHabitatEnhancement[0]['Total habitat area']).toFixed(2)).toEqual('0.40')
    expect(response.d3OffSiteHabitatEnhancement[0]['Distinctiveness change']).toEqual('Low - Low')
    expect(response.d3OffSiteHabitatEnhancement[0]['Condition change']).toEqual('Moderate - Moderate')
