...
const handlers = {
  ...
  post: async (request, h) => {
    ...
    const landBoundaryLocation = request.yar.get(constants.cacheKeys.LAND_BOUNDARY_LOCATION)
    request.yar.set(constants.cacheKeys.LAND_BOUNDARY_CHECKED, checkLandBoundary)
    if (checkLandBoundary === 'no') {
      ...
      return h.redirect((request.yar.get(constants.cacheKeys.LAND_BOUNDARY_GRID_REFERENCE) && request.yar.get(constants.cacheKeys.REFERER, true)) ||
...
const getContext = request => {
  ...
  const fileSize = request.yar.get(constants.cacheKeys.LAND_BOUNDARY_FILE_SIZE)
