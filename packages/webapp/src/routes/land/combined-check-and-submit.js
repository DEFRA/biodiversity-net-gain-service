...
const handlers = {
  get: async (request, h) => {
    ...
    return request.yar.get(constants.redisKeys.APPLICATION_REFERENCE) !== undefined &&
      ...
      ? h.view(constants.views.COMBINED_CHECK_AND_SUBMIT, {
        ...getContext(request)
      })
      : h.redirect('/')
  },
  post: async (request, h) => {
    ...
    const result = await postJson(`${constants.AZURE_FUNCTION_APP_URL}/processcombinedapplication`, value)
...
export default [{
  method: 'GET',
  path: constants.routes.COMBINED_CHECK_AND_SUBMIT,
