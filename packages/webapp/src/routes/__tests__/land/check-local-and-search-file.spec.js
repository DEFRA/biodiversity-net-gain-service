...
const url = constants.routes.CHECK_LOCAL_AND_SEARCH_FILE
...
describe(url, () => {
  ...
  describe('POST', () => {
    ...
    it('should allow confirmation that the correct local search file has been uploaded', async () => {
      postOptions.payload.checkLocalandSearch = constants.confirmLegalAgreementOptions.YES
      await submitPostRequest(postOptions)
    })

    it('should allow an alternative local search file to be uploaded ', async () => {
      const spy = jest.spyOn(azureStorage, 'deleteBlobFromContainers')
      postOptions.payload.checkLocalandSearch = 'no'
