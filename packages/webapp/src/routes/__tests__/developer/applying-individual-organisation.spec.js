  ...
  describe('GET', () => {
    ...
    it(`should render the ${url.substring(1)} view with individual selected`, async () => {
      jest.isolateModules(async () => {
        cacheMap.set(constants.cacheKeys.DEVELOPER_LANDOWNER_TYPE, constants.individualOrOrganisationTypes.INDIVIDUAL)
    ...
    it(`should render the ${url.substring(1)} view with organisation selected`, async () => {
      jest.isolateModules(async () => {
        cacheMap.set(constants.cacheKeys.DEVELOPER_LANDOWNER_TYPE, constants.individualOrOrganisationTypes.ORGANISATION)
        let viewResult, contextResult
        const applicationByIndividualOrOganisation = require('../../developer/applying-individual-organisation.js')
        const request = {
          yar: cacheMap
