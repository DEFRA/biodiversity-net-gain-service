...
const getApplicationDetails = (session) => {
  ...
  return {
    dueDiligence: {
      ...
      checkDefraAccount: session.get(creditsPurchaseConstants.redisKeys.CREDITS_PURCHASE_DEFRA_ACCOUNT_DETAILS_CONFIRMED) && 'Confirmed',
...
const handlers = {
  get: (request, h) => {
    request.yar.clear(creditsPurchaseConstants.redisKeys.CREDITS_PURCHASE_CUSTOMER_DUE_DILIGENCE)
  ...
  post: async (request, h) => {
    request.yar.set(creditsPurchaseConstants.redisKeys.CREDITS_PURCHASE_CUSTOMER_DUE_DILIGENCE, true)
