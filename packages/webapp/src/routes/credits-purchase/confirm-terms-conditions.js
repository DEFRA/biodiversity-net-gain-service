...
const handlers = {
  get: async (request, h) => {
    const confirmed = request.yar.get(creditsPurchaseConstants.cacheKeys.CREDITS_PURCHASE_TERMS_AND_CONDITIONS_CONFIRMED) === 'true'
  ...
  post: async (request, h) => {
    ...
    if (!consent) {
      ...
      request.yar.set(creditsPurchaseConstants.cacheKeys.CREDITS_PURCHASE_TERMS_AND_CONDITIONS_CONFIRMED, consent)
