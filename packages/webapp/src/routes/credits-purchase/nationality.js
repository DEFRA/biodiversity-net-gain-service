...
const handlers = {
  ...
  post: (request, h) => {
    ...
    if (Object.values(nationalities).some(nationality => nationality !== '')) {
      ...
      return h.redirect(request.yar.get(creditsPurchaseConstants.redisKeys.REFERER, true) || creditsPurchaseConstants.routes.CREDITS_PURCHASE_CUSTOMER_DUE_DILIGENCE)
    } else {
      return h.view(creditsPurchaseConstants.views.CREDITS_PURCHASE_NATIONALITY, {
