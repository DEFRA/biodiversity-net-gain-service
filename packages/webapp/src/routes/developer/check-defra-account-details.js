...
const handlers = {
  get: async (request, h) => {
    ...
    request.yar.get(constants.cacheKeys.DEVELOPER_DEFRA_ACCOUNT_DETAILS_CONFIRMED, true)
    request.yar.get(constants.cacheKeys.ORGANISATION_ID, true)
  ...
  post: async (request, h) => {
    ...
    if (defraAccountDetailsConfirmed) {
      request.yar.set(constants.cacheKeys.DEVELOPER_DEFRA_ACCOUNT_DETAILS_CONFIRMED, defraAccountDetailsConfirmed)
...
const redirect = (session, h) => {
  if (session.get(constants.cacheKeys.DEVELOPER_IS_AGENT) === constants.APPLICANT_IS_AGENT.YES) {
    return constants.routes.DEVELOPER_LANDOWNER_OR_LEASEHOLDER
  } else if (session.get(constants.cacheKeys.DEVELOPER_LANDOWNER_OR_LEASEHOLDER) === constants.DEVELOPER_IS_LANDOWNER_OR_LEASEHOLDER.YES) {
