...
const downloadFile = async (request, h) => {
  const blobName = request.yar.get(constants.redisKeys.DEVELOPER_PLANNING_DECISION_NOTICE_LOCATION)
  const config = {
    blobName,
...
export default {
  method: 'GET',
  path: constants.routes.DEVELOPER_DOWNLOAD_PLANNING_DECISION_FILE,
