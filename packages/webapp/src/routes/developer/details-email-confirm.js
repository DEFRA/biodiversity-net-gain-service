...
const handlers = {
  get: async (request, h) => {
    ...
    return h.view(constants.views.DETAILS_EMAIL_CONFIRM, {
      emailAddress
    })
  },
  post: async (request, h) => {
    ...
    if (request.payload.correctEmail === 'yes') {
      ...
      if (!emailValidationError) {
        ...
        viewPage = h.view(constants.views.DETAILS_EMAIL_CONFIRM, { errorMessage: emailValidationError.err[0].text, selected: true })
      }
    }
    return viewPage
  }
...
export default [{
  method: 'GET',
  path: constants.routes.DETAILS_EMAIL_CONFIRM,
