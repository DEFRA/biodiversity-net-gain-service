...
describe('section-id-handler', () => {
  ...
  it('Should not redirect if path does not include /public/', async () => {
  ...
  it('Should store journey-start-section-id query param and remove it from url', async () => {
    const application = JSON.parse(testApplication.dataString)
  ...
  it('Should redirect to url with hash and clear value of JOURNEY_START_SECTION_ID if path is combined case check and submit and journey-start-section-id is present', async () => {
    const application = JSON.parse(testApplication.dataString)
  ...
  it('Should only redirect if path is combined case check and submit', async () => {
    const application = JSON.parse(testApplication.dataString)
    application[constants.redisKeys.JOURNEY_START_SECTION_ID] = 'test-section-id'
    const response = await submitGetRequest({ url: constants.reusedRoutes.COMBINED_CASE_CHECK_PLANNING_DECISION_NOTICE_FILE }, 200, application)
    expect(forwardedUrl(response).pathname).toBeUndefined()
